...
class EditLink extends StatefulWidget {
  ...
  const EditLink({super.key, required this.contextPage, required this.profile,
      required this.isEditLink, required this.linkController,
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Row(
          children: [
            const Text('Liên kết', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700),),
            const Spacer(),
            TextButton(
              onPressed: (){ 
                widget.setIsEditLink();
              }, 
              child: Text(widget.isEditLink ? 'Đặt lại' : 'Chỉnh sửa', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w400, color: Colors.blue),),
            ),
          ],
        ),
        ...
        Row(
          children: [
            ...
            widget.isEditLink
              ...
              : Flexible(
                  child: Text(widget.profile.link, style: const TextStyle(fontSize: 18),)
                ),
          ],
        ),
        const SizedBox(height: 10,),
